<script lang="ts">
  ...
  import { atlasFeed, pixelFeed, session, atlasAgentInfo, pixelAgentInfo, clearFeed } from '../stores/session';
  ...
  let { target }: { target: 'atlas' | 'pixel' } = $props();
  ...
  let rawFeed = $derived(target === 'atlas' ? $atlasFeed : $pixelFeed);
  // Apply local clear: show only items added after the last clear
  let feed = $derived(rawFeed.slice(clearedAt));

  let isRunning = $derived(target === 'atlas' ? $session.atlasRunning : $session.pixelRunning);

  let agentName = $derived(target === 'atlas' ? 'Atlas' : 'Pixel');
  let agentTag = $derived(target === 'atlas' ? 'Backend' : 'Frontend');
  let agentInfo = $derived(target === 'atlas' ? $atlasAgentInfo : $pixelAgentInfo);
