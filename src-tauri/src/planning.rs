...
fn call_claude_with_updates(
    ...
    let mut child_proc: Option<std::process::Child> = if is_local {
        ...
        let repo_path_alt = repo_path.replace("LocalCenter", "Localcenter");
        let bash_cmd = format!(
            "cat | ssh -o ServerAliveInterval=30 -o ServerAliveCountMax=20 {} \"unset CLAUDECODE; cd {} 2>/dev/null || cd {} 2>/dev/null || cd ~; claude -p --output-format text 2>/dev/null\" > {output_file}; touch {done_file}",
            shell_escape(&ssh_host), shell_escape(&repo_path), shell_escape(&repo_path_alt)
        );
        let mut proc = Command::new("bash")
            .args(["-c", &bash_cmd])
            .stdin(Stdio::piped())
...
fn get_remote_repo_status(host: &str, repo_path: &str) -> RepoStatus {
    ...
    let script = format!(
        "cd {} 2>/dev/null || cd {} 2>/dev/null; \
         ...
         echo ===END===",
        shell_escape(repo_path), shell_escape(&repo_path_alt)
    );
    ...
    for (_id, host, repo_path, is_local) in &repos {
        if *is_local {
            ...
            front_ok = Command::new("bash")
                .args(["-c", &format!(
                    "ssh -o ServerAliveInterval=30 {} 'cd {} 2>/dev/null || cd {} 2>/dev/null; git checkout -b {} 2>/dev/null || git checkout {} 2>/dev/null'",
                    shell_escape(host), shell_escape(repo_path), shell_escape(&repo_path_alt),
...
pub fn get_repo_branches(app: AppHandle, repo: String) -> Vec<String> {
    ...
    if repo == "back" {
        ...
        let output = Command::new("bash")
            .args(["-c", &format!(
                "ssh -o ConnectTimeout=5 {} 'cd {} 2>/dev/null || cd {} 2>/dev/null; git branch -a --sort=-committerdate --format=\"%(refname:short)\" 2>/dev/null'",
                shell_escape(host), shell_escape(repo_path), shell_escape(&repo_path_alt)
            )])
            .output()
            .map(|o| String::from_utf8_lossy(&o.stdout).to_string())
