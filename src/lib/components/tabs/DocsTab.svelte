  ...
  async function startAnalysis() {
    ...
    try {
      ...
      try {
        ...
        repoInfo = statuses.map((s, i) => {
          const mid = machineIds[i] ?? `machine-${i}`;
          const name = ($machines[mid]?.name ?? mid).toUpperCase();
          return `${name}: branch=${s.branch}, changed=${s.changed}, staged=${s.staged}, ahead=${s.ahead}, behind=${s.behind}, lastCommit="${s.lastCommit}"`;
        }).join('\n');
...
TASK|<machine-id>|descripcion concreta de lo que hay que hacer

Maquinas disponibles:
${Object.entries($machines).map(([id, m]) => `- ${id} = ${m.name} (${m.role || id})`).join('\n')}
...
- Responde SOLO con las lineas TASK|target|descripcion, nada mas`;
      ...
      const defaultTarget = Object.keys($machines)[0] ?? 'atlas';
      const task = await apiSendTask(defaultTarget, prompt);
        ...
        <div class="steps-list">
          {#each planSteps as step, i}
            <div class="step-row">
              ...
              <span class="step-target" >{step.target.toUpperCase()}</span>
