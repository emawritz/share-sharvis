<script lang="ts">
  ...
  let agentStatus = $state<Record<string, {running: boolean, task: string, elapsed: number}>>({
    atlas: { running: false, task: '', elapsed: 0 },
    pixel: { running: false, task: '', elapsed: 0 },
  });
  ...
  // Derives only from tasks — re-runs on task changes, not every second tick
  let runningAtlas = $derived($tasks.find(t => t.status === 'running' && t.target === 'atlas'));
  let runningPixel = $derived($tasks.find(t => t.status === 'running' && t.target === 'pixel'));

  // Only tick the clock when at least one agent is running
  $effect(() => {
    const running = runningAtlas || runningPixel;
    if (!running) return;
  ...
  $effect(() => {
    agentStatus = {
      atlas: {
        running: !!runningAtlas,
        task: runningAtlas?.prompt.slice(0, 50) || '',
        elapsed: runningAtlas ? Math.floor((now - (runningAtlas.startedAt ?? now)) / 1000) : 0,
      },
      pixel: {
        running: !!runningPixel,
        task: runningPixel?.prompt.slice(0, 50) || '',
        elapsed: runningPixel ? Math.floor((now - (runningPixel.startedAt ?? now)) / 1000) : 0,
      },
    };
  });
...
<div class="mc-panel">
  ...
  <div class="mc-card">
    ...
    {#each ['atlas', 'pixel'] as agent}
      {@const a = agentStatus[agent]}
      <div class="mc-agent">
        <span class="mc-dot" class:mc-dot-active={a.running} class:mc-dot-idle={!a.running}></span>
        <span class="mc-agent-name">{agent.toUpperCase()}</span>
        {#if a.running}
