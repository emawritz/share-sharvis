...
pub fn repo_path_to_jsonl_dir(repo_path: &str, is_local: bool, remote_home: Option<&str>) -> String {
    let expanded = if let Some(rest) = repo_path.strip_prefix("~/") {
        if is_local {
            ...
            let rh = remote_home.unwrap_or("/home/user");
            format!("{}/{}", rh, rest)
        }
    } else {
        repo_path.to_string()
...
mod tests {
    ...
    fn jsonl_dir_local_absolute_path() {
        // Local absolute path: /Users/testuser/myproject
        // Expected: <home>/.claude/projects/-Users-testuser-myproject/
        let result = repo_path_to_jsonl_dir("/Users/testuser/myproject", true, None);
        let home = dirs::home_dir()
            .unwrap_or_default()
            .to_string_lossy()
            .to_string();
        let expected = format!("{}/.claude/projects/-Users-testuser-myproject/", home);
    ...
    fn jsonl_dir_remote_tilde_path_default_home() {
        // Remote path with ~ but no explicit home_dir — should use default "/home/user"
        let result = repo_path_to_jsonl_dir("~/myproject", false, None);
        let expected = "/home/user/.claude/projects/-home-user-myproject/";
        assert_eq!(result, expected);
    }
    ...
    fn jsonl_dir_slashes_become_dashes() {
        ...
        let result = repo_path_to_jsonl_dir("/Users/jane/my-app", false, Some("/home/testuser"));
        // expanded: /Users/jane/my-app  (absolute, no ~ expansion)
        // dir_name: Users-jane-my-app
        let expected = "/home/testuser/.claude/projects/-Users-jane-my-app/";
        assert_eq!(result, expected);
    }
