<script lang="ts">
  ...
  import { session, atlasFeed, pixelFeed } from '$lib/stores/session';
  ...
  // Combined recent activity from atlas + pixel feeds (last 5)
  let recentActivity = $derived.by(() => {
    const combined: (Activity & { source: 'atlas' | 'pixel' })[] = [
      ...$atlasFeed.map((a: Activity) => ({ ...a, source: 'atlas' as const })),
      ...$pixelFeed.map((a: Activity) => ({ ...a, source: 'pixel' as const })),
    ];
  ...
  let services: ServiceStatus[] = $state([
    ...
    // { name: 'GPU Dashboard', port: 7777, host: '10.0.0.10', status: 'checking' },
  ]);
