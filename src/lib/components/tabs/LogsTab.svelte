<script lang="ts">
  import { machineFeed, clearFeed } from '../../stores/session';
  ...
  // Agent selector — dynamic machine ID
  let machineIds = $derived(Object.keys($machines));
  let selectedAgent = $state('');

  // Auto-select first machine when available
  $effect(() => {
    if (!selectedAgent && machineIds.length > 0) selectedAgent = machineIds[0];
  });
  ...
  let rawFeed = $derived($machineFeed[selectedAgent] ?? []);
      ...
      {#if mode === 'agent'}
        ...
        <div class="agent-toggle">
          {#each machineIds as mid}
            <button
              class="agent-btn"
              class:active={selectedAgent === mid}
              onclick={() => selectedAgent = mid}
            >{($machines[mid]?.name ?? mid).toUpperCase()}</button>
          {/each}
        </div>
      {/if}
