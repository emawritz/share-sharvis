<script lang="ts">
  ...
  function confirmAddPreset(name: string) {
    ...
    const presetTarget: Preset['target'] =
      target === 'atlas' || target === 'pixel' || target === 'both' ? target : 'both';
  ...
  function toggleChain() {
    ...
    if (chainOpen && chainSteps.length === 0) {
      chainSteps = [{ target: 'atlas', prompt: '', runCondition: 'on_success' }];
    }
  }

  function addChainStep() {
    chainSteps = [...chainSteps, { target: 'atlas', prompt: '', runCondition: 'on_success' }];
  }
